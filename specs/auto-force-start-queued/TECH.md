...
The design is based on fork commit
[`45cd84a`](https://github.com/calmcacil/torboxarr/tree/45cd84a02341db7ff77aa502a5e48b3e1fc906d0):

- [`internal/worker/poll.go:55-81`](https://github.com/calmcacil/torboxarr/blob/45cd84a02341db7ff77aa502a5e48b3e1fc906d0/internal/worker/poll.go#L55-L81)
  checks the queued view first and has the successful queue confirmation needed
  to gate force-start safely.
- [`internal/worker/poll.go:128-155`](https://github.com/calmcacil/torboxarr/blob/45cd84a02341db7ff77aa502a5e48b3e1fc906d0/internal/worker/poll.go#L128-L155)
  persists a confirmed queued status and schedules the next poll.
- [`internal/torbox/tasks.go:302-318`](https://github.com/calmcacil/torboxarr/blob/45cd84a02341db7ff77aa502a5e48b3e1fc906d0/internal/torbox/tasks.go#L302-L318)
  calls TorBox's queued service and normalizes torrent and Usenet queue types.
- [`internal/torbox/client.go:9-18`](https://github.com/calmcacil/torboxarr/blob/45cd84a02341db7ff77aa502a5e48b3e1fc906d0/internal/torbox/client.go#L9-L18)
  defines the worker-facing TorBox client contract.
- [`internal/store/model.go:46-68`](https://github.com/calmcacil/torboxarr/blob/45cd84a02341db7ff77aa502a5e48b3e1fc906d0/internal/store/model.go#L46-L68)
  stores worker bookkeeping in `metadata_json` without schema changes.
- [`internal/store/store.go:190-207`](https://github.com/calmcacil/torboxarr/blob/45cd84a02341db7ff77aa502a5e48b3e1fc906d0/internal/store/store.go#L190-L207)
  provides state-conditional, removal-safe updates.
- [`internal/config/config.go:71-83`](https://github.com/calmcacil/torboxarr/blob/45cd84a02341db7ff77aa502a5e48b3e1fc906d0/internal/config/config.go#L71-L83)
  owns worker policy configuration.
...
Add `Workers.QueuedForceStartAfter time.Duration` with a default of three
hours. Read it from `TORBOXARR_QUEUED_FORCE_START_AFTER` using `time.ParseDuration`.
...
the setting in `.env.example` and `README.md`.
...
Before coding the body, capture the exact request generated by TorBox's current
Web UI or read the authoritative OpenAPI schema. Add an HTTP contract test that
asserts the method, path, content type, operation, queue-ID field type, and any
required source-type field. Do not infer the request schema from the existing
active-task delete bodies.
...
Change `keepQueued` to perform these steps:
...
The existing claim mechanism prevents ordinary overlapping pollers from
calling force-start concurrently. It cannot make the external API request and
SQLite update atomic. A crash after TorBox accepts the request but before the
metadata update therefore remains an uncertain outcome and may produce one
retry after restart, as allowed by Product Behavior 28.
...
If stronger cross-process exclusion is required later, add an atomic
force-start lease to the store. It is not warranted for the current single
service deployment and would not eliminate the external-call crash window.
...
Persisted metadata provides existing database diagnostics for Product Behavior
36. Do not add a new Arr-compatible state or public endpoint in the initial
implementation.
...
- Default is `3h` and satisfies Product Behavior 1 and 39.
- `0` disables requests (Behavior 3 and 38).
- Positive Go duration values parse (Behavior 2 and 37).
- Negative and malformed values fail configuration loading with the variable
  named in the error (Behavior 37).
...
- no request before three hours, one request at/after three hours (Behavior 1,
  22);
- disabled configuration (Behavior 3 and 38);
...
- structured logs do not contain configured secrets (Behavior 34-35).
...
- **Unexpected default behavior after upgrade:** the three-hour default is an
  explicit product decision; document `0` as the opt-out prominently.
...
- **Historical jobs immediately force-starting:** initialize recovered lifecycle
  ...
  local job age.
- **TorBox slot pressure:** wait for the documented three-hour queue period and
  send one item-specific request per eligible lifecycle.
...
coordination overhead. Implement sequentially on
